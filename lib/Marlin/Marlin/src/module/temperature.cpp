...
void Temperature::PID_autotune(const float &target, const heater_ind_t heater, const int8_t ncycles, const bool set_result /*=false*/) {
    ...
    while (wait_for_heatup) {
            ...
            if (
                ...
                if (!heated) { // If not yet reached target...
                    if (current_temp > next_watch_temp) { // Over the watch temp?
                        ...
                        if (current_temp > watch_temp_target) {
        ...
        if (cycles > ncycles && cycles > 2) {
            ...
            say_default_();
            serialprintPGM(estring);
            SERIAL_ECHOLNPAIR("Kp ", tune_pid.Kp);
            say_default_();
            serialprintPGM(estring);
            SERIAL_ECHOLNPAIR("Ki ", tune_pid.Ki);
            say_default_();
            serialprintPGM(estring);
            SERIAL_ECHOLNPAIR("Kd ", tune_pid.Kd);
        ...
        #else
            say_default_();
            SERIAL_ECHOLNPAIR("bedKp ", tune_pid.Kp);
            say_default_();
            SERIAL_ECHOLNPAIR("bedKi ", tune_pid.Ki);
            say_default_();
            SERIAL_ECHOLNPAIR("bedKd ", tune_pid.Kd);
        #endif
...
void Temperature::checkExtruderAutoFans() {
    ...
    #if HAS_AUTO_FAN_1
        case 1:
            _UPDATE_AUTO_FAN(E1, fan_on, EXTRUDER_AUTO_FAN_SPEED);
            break;
    #endif
    #if HAS_AUTO_FAN_2
        case 2:
            _UPDATE_AUTO_FAN(E2, fan_on, EXTRUDER_AUTO_FAN_SPEED);
            break;
    #endif
    #if HAS_AUTO_FAN_3
        case 3:
            _UPDATE_AUTO_FAN(E3, fan_on, EXTRUDER_AUTO_FAN_SPEED);
            break;
    #endif
    #if HAS_AUTO_FAN_4
        case 4:
            _UPDATE_AUTO_FAN(E4, fan_on, EXTRUDER_AUTO_FAN_SPEED);
            break;
    #endif
    #if HAS_AUTO_FAN_5
        case 5:
            _UPDATE_AUTO_FAN(E5, fan_on, EXTRUDER_AUTO_FAN_SPEED);
            break;
    #endif
...
void Temperature::_temp_error(const heater_ind_t heater, PGM_P const serial_msg, PGM_P const lcd_msg) {
    ...
    if (IsRunning()
        ...
        if (heater >= 0) {
            ...
            SERIAL_ECHOPGM(MSG_HEATER_CHAMBER);
        }
#endif
        else {
            SERIAL_ECHOPGM(MSG_HEATER_BED);
        }
        SERIAL_EOL();
    }
...
void Temperature::manage_heater() {
    ...
    if (temp_hotend[0].celsius > _MIN(HEATER_0_MAXTEMP, HEATER_0_MAX6675_TMAX - 1.0)) {
        max_temp_error(H_E0);
    }
    if (temp_hotend[0].celsius < _MAX(HEATER_0_MINTEMP, HEATER_0_MAX6675_TMIN + .01)) {
        min_temp_error(H_E0);
    }
#endif

#if ENABLED(HEATER_1_USES_MAX6675)
    if (temp_hotend[1].celsius > _MIN(HEATER_1_MAXTEMP, HEATER_1_MAX6675_TMAX - 1.0)) {
        max_temp_error(H_E1);
    }
    if (temp_hotend[1].celsius < _MAX(HEATER_1_MINTEMP, HEATER_1_MAX6675_TMIN + .01)) {
        min_temp_error(H_E1);
    }
#endif
     ...
     * (I2C sensor path handles this in the planner instead.)
     */
...
float Temperature::user_thermistor_to_deg_c(const uint8_t t_index, const int raw) {
    ...
    if (!WITHIN(t_index, 0, COUNT(user_thermistor) - 1)) {
        return 25;
    }
...
void Temperature::updateTemperaturesFromRawValues() {
    ...
    #if ENABLED(PRUSA_TOOLCHANGER)
    HOTEND_LOOP()
    temp_hotend[e].celsius
        = prusa_toolchanger.getTool(e).get_hotend_temp();
    #else
    HOTEND_LOOP()
    temp_hotend[e].celsius
        = analog_to_celsius_hotend(temp_hotend[e].raw, e);
    #endif
#endif
#if HAS_HEATED_BED
    #if ENABLED(MODULAR_HEATBED)
    ...
    #if ENABLED(PRUSA_TOOLCHANGER)
    HOTEND_LOOP()
    temp_heatbreak[e].celsius
        = prusa_toolchanger.getTool(e).get_heatbreak_temp();
    #else
    HOTEND_LOOP()
    temp_heatbreak[e].celsius
        = analog_to_celsius_heatbreak(temp_heatbreak[e].raw);
    #endif
#endif
#if ENABLED(TEMP_SENSOR_1_AS_REDUNDANT)
...
#define INIT_FAN_PIN(P)       \
    ...
    #define INIT_E_AUTO_FAN_PIN(P)                         \
        do {                                               \
            if (P == FAN1_PIN || P == FAN2_PIN) {          \
                SET_PWM(P);                                \
                SET_FAST_PWM_FREQ(FAST_PWM_FAN_FREQUENCY); \
            } else                                         \
                SET_OUTPUT(P);                             \
        } while (0)
#else
...
void Temperature::init() {
    ...
    #endif
    #ifdef BED_MAXTEMP
    while (analog_to_celsius_bed(maxtemp_raw_BED) > BED_MAXTEMP) {
        maxtemp_raw_BED -= TEMPDIR(BED) * (OVERSAMPLENR);
    }
    #endif
#endif // HAS_HEATED_BED
...
#if HAS_HEATED_CHAMBER
    #ifdef CHAMBER_MINTEMP
    while (analog_to_celsius_chamber(mintemp_raw_CHAMBER) < CHAMBER_MINTEMP) {
        mintemp_raw_CHAMBER += TEMPDIR(CHAMBER) * (OVERSAMPLENR);
    }
    #endif
    #ifdef CHAMBER_MAXTEMP
    while (analog_to_celsius_chamber(maxtemp_raw_CHAMBER) > CHAMBER_MAXTEMP) {
        maxtemp_raw_CHAMBER -= TEMPDIR(CHAMBER) * (OVERSAMPLENR);
    }
    #endif
#endif
    ...
    while (analog_to_celsius_heatbreak(mintemp_raw_HEATBREAK) < HEATBREAK_MINTEMP) {
        mintemp_raw_HEATBREAK += TEMPDIRHEATBREAK * (OVERSAMPLENR);
    }
    #endif
    #ifdef HEATBREAK_MAXTEMP
    while (analog_to_celsius_heatbreak(maxtemp_raw_HEATBREAK) > HEATBREAK_MAXTEMP) {
        maxtemp_raw_HEATBREAK -= TEMPDIRHEATBREAK * (OVERSAMPLENR);
    }
    #endif
#endif
#if HAS_TEMP_BOARD
    #ifdef BOARD_MINTEMP
    while (analog_to_celsius_board(mintemp_raw_BOARD) < BOARD_MINTEMP) {
        mintemp_raw_BOARD += TEMPDIRBOARD * (OVERSAMPLENR);
    }
    #endif
    #ifdef BOARD_MAXTEMP
    while (analog_to_celsius_board(maxtemp_raw_BOARD) > BOARD_MAXTEMP) {
        maxtemp_raw_BOARD -= TEMPDIRBOARD * (OVERSAMPLENR);
    }
    #endif
#endif
...
void Temperature::pause(const bool p) {
    if (p != paused) {
        ...
        if (p) {
            HOTEND_LOOP()
            hotend_idle[e].expire(); // timeout immediately
    #if HAS_HEATED_BED
            bed_idle.expire(); // timeout immediately
    #endif
        } else {
            HOTEND_LOOP()
            reset_heater_idle_timer(e);
    #if HAS_HEATED_BED
...
int Temperature::read_max6675(
    ...
    #if COUNT_6675 > 1
        #define WRITE_MAX6675(V)               \
            do {                               \
                switch (hindex) {              \
                case 1:                        \
                    WRITE(MAX6675_SS2_PIN, V); \
                    break;                     \
                default:                       \
                    WRITE(MAX6675_SS_PIN, V);  \
                }                              \
            } while (0)
        #define SET_OUTPUT_MAX6675()             \
            do {                                 \
                switch (hindex) {                \
                case 1:                          \
                    SET_OUTPUT(MAX6675_SS2_PIN); \
                    break;                       \
                default:                         \
                    SET_OUTPUT(MAX6675_SS_PIN);  \
                }                                \
            } while (0)
    #elif ENABLED(HEATER_1_USES_MAX6675)
    ...
    for (uint8_t i = sizeof(max6675_temp); i--;) {
        ...
        if (i > 0) {
            max6675_temp <<= 8; // shift left if not the last byte
        }
    }
    ...
    if (max6675_temp & 0x00002000) {
        max6675_temp |= 0xFFFFC000; // Support negative temperature
    }
    #endif
...
// Filament Sensor - update filtered values for ADC-based sensors
    ...
    if (CHAMBERCMP(temp_chamber.raw, maxtemp_raw_CHAMBER)) {
        max_temp_error(H_CHAMBER);
    }
    if (chamber_on && CHAMBERCMP(mintemp_raw_CHAMBER, temp_chamber.raw)) {
    ...
    HOTEND_LOOP() {
        ...
        if (HEATBREAKCMP(temp_heatbreak[e].raw, maxtemp_raw_HEATBREAK)) {
            max_temp_error(static_cast<heater_ind_t>(H_HEATBREAK_E0 + e));
        }
        if (heater_on && HEATBREAKCMP(mintemp_raw_HEATBREAK, temp_heatbreak[e].raw)) {
            min_temp_error(static_cast<heater_ind_t>(H_HEATBREAK_E0 + e));
        }
    #endif
    }
#endif
...
void Temperature::isr() {
    ...
    if (pwm_count_tmp >= 127) {
        ...
        #if ENABLED(FAN_SOFT_PWM)
            #if HAS_FAN0
        if (soft_pwm_count_fan[0] <= pwm_count_tmp) {
            WRITE_FAN(0, LOW);
        }
            #endif
            #if HAS_FAN1
        if (soft_pwm_count_fan[1] <= pwm_count_tmp) {
            WRITE_FAN(1, LOW);
        }
            #endif
            #if HAS_FAN2
        if (soft_pwm_count_fan[2] <= pwm_count_tmp) {
            WRITE_FAN(2, LOW);
        }
            #endif
        #endif
    ...
    pwm_count = pwm_count_tmp + _BV(SOFT_PWM_SCALE);
         ...
         */
        #define _SLOW_SET(NR, PWM, V)     \
            do {                          \
                if (PWM.ready(V))         \
                    WRITE_HEATER_##NR(V); \
            } while (0)
        #define _SLOW_PWM(NR, PWM, SRC)              \
            do {                                     \
                PWM.count = SRC.soft_pwm_amount;     \
                _SLOW_SET(NR, PWM, (PWM.count > 0)); \
            } while (0)
        #define _PWM_OFF(NR, PWM)               \
            do {                                \
                if (PWM.count < slow_pwm_count) \
                    _SLOW_SET(NR, PWM, 0);      \
            } while (0)
...
#if HAS_TEMP_HEATBREAK
    case PrepareTemp_HEATBREAK:
        HAL_START_ADC(TEMP_HEATBREAK_PIN);
        break;
    case MeasureTemp_HEATBREAK:
        ACCUMULATE_ADC(temp_heatbreak[0]);
        break;
#endif

#if HAS_TEMP_BOARD
    case PrepareTemp_BOARD:
        HAL_START_ADC(TEMP_BOARD_PIN);
        break;
    case MeasureTemp_BOARD:
        ACCUMULATE_ADC(temp_board);
        break;
#endif

#if PRINTER_IS_PRUSA_iX()
    case PrepareTemp_PSU:
        HAL_START_ADC(TEMP_PSU_PIN);
        break;
    case MeasureTemp_PSU:
        ACCUMULATE_ADC(temp_psu);
        break;
    case PrepareTemp_AMBIENT:
        HAL_START_ADC(TEMP_AMBIENT_PIN);
        break;
    case MeasureTemp_AMBIENT:
        ACCUMULATE_ADC(temp_ambient);
        break;
#endif
...
void Temperature::print_heater_states(const uint8_t target_extruder
    ...
    #if HAS_TEMP_HEATBREAK
    print_heater_state(degHeatbreak(target_extruder), degTargetHeatbreak(target_extruder)
        #if ENABLED(SHOW_TEMP_ADC_VALUES)
                                                          ,
        rawHeatbreakTemp()
        #endif
...
bool Temperature::wait_for_hotend(const uint8_t target_extruder, const bool no_wait_for_cooling /*=true*/, bool fan_cooling /*=false*/
    ...
    do {
        ...
        if (!wants_to_cool) {
            printerEventLEDs.onHotendHeating(start_temp, temp, target_temp);
        }
        #endif
        ...
        if (wants_to_cool) {
            ...
            if (!next_cool_check_ms || ELAPSED(now, next_cool_check_ms)) {
                if (old_temp - temp < float(MIN_COOLING_SLOPE_DEG)) {
                    break;
                }
    ...
    do {
        ...
        if (!residency_start_ms) {
            ...
            if (temp_diff < TEMP_BED_WINDOW) {
                ...
                if (first_loop) {
                    residency_start_ms += (TEMP_BED_RESIDENCY_TIME) * 1000UL;
                }
            }
        ...
        if (wants_to_cool) {
            ...
            if (!next_cool_check_ms || ELAPSED(now, next_cool_check_ms)) {
                if (old_temp - temp < float(MIN_COOLING_SLOPE_DEG_BED)) {
                    break;
                }
    ...
    do {
        ...
        if (!residency_start_ms) {
            ...
            if (temp_diff < TEMP_CHAMBER_WINDOW) {
                residency_start_ms = now;
                if (first_loop) {
                    residency_start_ms += (TEMP_CHAMBER_RESIDENCY_TIME) * 1000UL;
                }
            }
        ...
        if (wants_to_cool) {
            ...
            if (!next_cool_check_ms || ELAPSED(now, next_cool_check_ms)) {
                if (old_temp - temp < float(MIN_COOLING_SLOPE_DEG_CHAMBER)) {
                    break;
                }
