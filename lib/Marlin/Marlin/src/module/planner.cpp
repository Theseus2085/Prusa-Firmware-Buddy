...
void Planner::reverse_pass_kernel(block_t *const previous, block_t *const current, const block_t *const next OPTARG(HINTS_SAFE_EXIT_SPEED, const_float_t safe_exit_speed_sqr)) {
...
void Planner::getHighESpeed() {
    ...
    if (thermalManager.degTargetHotend(0) + 2 < autotemp_min) {
    ...
    if (t < oldt) {
        t = t * (1 - float(AUTOTEMP_OLDWEIGHT)) + oldt * float(AUTOTEMP_OLDWEIGHT);
    }
    oldt = t;
...
void Planner::check_axes_activity() {
    ...
    if (const block_t *block = get_first_move_block(); block != nullptr) {
        ...
        for (uint8_t b = block_buffer_tail; b != block_buffer_head; b = next_block_index(b)) {
            block_t *block = &block_buffer[b];
            LOOP_XYZE(i)
            if (block->msteps[i]) axis_active[i] = true;
        }
#endif
    } else {
...
//
    ...
    if (!axis_active.x && !axis_active.y) {
        disable_XY();
    }
#else
...
void Planner::apply_filament_width_sensor(const float encoded_ratio) {
    // Reconstitute the nominal/actual area correction ratio.
    const float area_correction_ratio = 1 + 0.01f * encoded_ratio;

    volumetric_multiplier[FILAMENT_SENSOR_EXTRUDER_NUM] = parser.volumetric_enabled
        ? area_correction_ratio / CIRCLE_AREA(filwidth.nominal_mm * 0.5f) // Volumetric scales by nominal-area reciprocal.
        : area_correction_ratio; // Linear scales E directly by area correction.
 ...
 */
bool Planner::_buffer_msteps_raw(const xyze_long_t &target, const xyze_pos_t &target_float, feedRate_t fr_mm_s, const uint8_t extruder, const PlannerHints &hints) {
 ...
 * Returns true if movement was properly queued, false otherwise
 */
bool Planner::_buffer_msteps(const xyze_long_t &target, const xyze_pos_t &target_float, feedRate_t fr_mm_s, const uint8_t extruder, const PlannerHints &hints) {
...
bool Planner::_populate_block(block_t *const block,
    const abce_long_t &target, const xyze_pos_t &target_float, feedRate_t fr_mm_s, const uint8_t extruder, const PlannerHints &hints) {
    ...
    #if ENABLED(FILAMENT_WIDTH_SENSOR)
    /*
     * Volumetric Extrusion Integration / Lookahead Averaging:
     * Sampling a momentary diameter value for a long G-Code segment causes low-frequency aliasing (over/underextrusion).
     * We mitigate this by calculating the length-weighted integral of the cross-sectional area
     * over the required physical extrusion length (abs_e_mm) using the synchronized delay buffer.
     *
     * Note: This adjustment updates the target `volumetric_multiplier` and `e_factor`
     * before they are used to compute `e_msteps_float`.
     */
...
// Enable extruder(s)
    ...
    if (e_msteps) {
        ...
        #define DISABLE_IDLE_E(N)            \
            if (!g_uc_extruder_last_move[N]) \
                disable_E##N();
...
// Slow down when the buffer starts to empty, rather than wait at the corner for a buffer refill
    ...
    LOOP_XY(i) {
        ...
        if (cs > settings.max_feedrate_mm_s[i]) {
            NOMORE(speed_factor, settings.max_feedrate_mm_s[i] / cs);
        }
    }
#endif
...
#define LIMIT_ACCEL_LONG(AXIS, INDX)                                                                      \
    do {                                                                                                  \
        if (block->msteps[AXIS] && max_acceleration_msteps_per_s2[AXIS + INDX] < accel) {                 \
            ...
            if (accel * block->msteps[AXIS] > comp)                                                       \
                accel = comp / block->msteps[AXIS];                                                       \
        }                                                                                                 \
    } while (0)
...
#define LIMIT_ACCEL_FLOAT(AXIS, INDX)                                                                                \
    do {                                                                                                             \
        if (block->msteps[AXIS] && max_acceleration_msteps_per_s2[AXIS + INDX] < accel) {                            \
            ...
            if ((float)accel * (float)block->msteps[AXIS] > comp)                                                    \
                accel = comp / (float)block->msteps[AXIS];                                                           \
        }                                                                                                            \
    } while (0)
 ...
 */
bool Planner::buffer_segment(const abce_pos_t &abce, const_feedRate_t fr_mm_s, const uint8_t extruder /*=active_extruder*/
    ,
...
// When changing extruders recalculate mini-steps corresponding to the E position
    ...
    if (!_buffer_msteps(target, abce, fr_mm_s, extruder
#if ENABLED(CRASH_RECOVERY)
            ,
            *segment_hints
#else
 ...
 */
bool Planner::buffer_line(const xyze_pos_t &cart, const_feedRate_t fr_mm_s, const uint8_t extruder /*=active_extruder*/
    ,
    const PlannerHints &hints /*=PlannerHints()*/
) {
...
void Planner::refresh_acceleration_rates() {
    ...
    LOOP_XYZE_N(i) {
        ...
        if (AXIS_CONDITION) {
            NOLESS(highest_rate, max_acceleration_msteps_per_s2[i]);
        }
    }
...
void Planner::autotemp_M104_M109() {
    if ((autotemp_enabled = parser.seen('F'))) {
        autotemp_factor = parser.value_float();
    }
    if (parser.seen('S')) {
        autotemp_min = parser.value_celsius();
    }
    if (parser.seen('B')) {
        autotemp_max = parser.value_celsius();
    }
}
